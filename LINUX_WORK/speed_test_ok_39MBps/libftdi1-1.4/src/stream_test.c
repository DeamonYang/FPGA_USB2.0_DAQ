...
{
	int i;
	static unsigned long long err_num = 0;
	static unsigned long long tot_len = 0;
	unsigned char temp;
   if (length)
   {
/*        if (check)
	   ...
	   if (check)
	   {
		   ...
		   printf("totlen %10.02f  MB ",tot_len/(1024*1024.0));
		   printf("toterr %d  curlen: %d \r\n",err_num,length);
	   
	   }
/*	   
	   printf(stderr, "%10.02fs total time %9.3f MiB captured %7.1f kB/s curr rate %7.1f kB/s totalrate %d dropouts\n",
          progress->totalTime,
...
/*   if(ftdi_usb_purge_rx_buffer(ftdi) < 0)
   ...
   err = ftdi_readstream(ftdi, readCallback, NULL, 8, 2);
   if (err < 0 && !exitRequested)
   {
	   printf("err cod %d",err);
	   exit(1);
